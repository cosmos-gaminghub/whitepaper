...
Gaming Hub はマルチアセット分散元帳であるが、quark と呼ばれる特別なネイティブトークンがある。 quarkはGaming Hubの唯一のステーキングトークンである。 quarkは、所有者がvalidatorに投票、検証、または委任するための権利を保有していることを表している。 また、Nbiru Hub上で流通するトークンには、leptonがある。これは、EthereumのETHのように、スパムを軽減するために取引手数料の支払いに使用することができる。ブロック報酬は、バリデータとバリデータに委任するデリゲータにleptonとして与えられる。
